...
import { Sync, Calculate, Warning, Visibility } from '@mui/icons-material'
...
const Admin = () => {
 ...
 // ─── Config ────────────────────────────────────
 const { data: showOthers } = useAppConfig('show_others_predictions')
   ...
   <Box sx={{ maxWidth: 800, mx: 'auto', px: 2, py: 3 }}>
     ...
     {tab === 0 && (
       <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
         ...
         {/* Toggle visibilidad de predicciones */}
         <Card>
           <CardContent sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 2 }}>
             <Box sx={{ display: 'flex', alignItems: 'flex-start', gap: 2 }}>
               <Box sx={{ color: '#00bfff', mt: 0.5 }}><Visibility /></Box>
               <Box>
                 <Typography fontWeight={700}>Predicciones visibles</Typography>
                 <Typography variant="body2" color="text.secondary">
                   {showOthers === 'true'
                     ? 'Los jugadores pueden ver las predicciones de todos.'
                     : 'Los jugadores solo ven su propia predicción.'}
                 </Typography>
               </Box>
             </Box>
             <Switch
               checked={showOthers === 'true'}
               onChange={(e) => updateConfig.mutate({
                 key: 'show_others_predictions',
             ...
             <CardContent>
               ...
               {missingPredictors.map(u => (
                 <Chip
                   ...
                   size="small"
                   sx={{ mr: 0.5, mb: 0.5, color: '#ffd700', background: 'rgba(255,215,0,0.1)' }}
                 />
               ))}

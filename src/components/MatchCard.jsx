...
const MatchCard = ({ match, prediction, onPredict }) => {
 ...
 const isET       = match.status === 'ET'
 const isPenalty  = match.status === 'P'
 ...
 const getStatusChip = () => {
   ...
   if (isPenalty)  return { label: '🥅 Penales',                    color: '#cc66ff', bg: 'rgba(204,102,255,0.15)',  live: true  }
   if (isET)       return { label: '⏱ Prórroga',                    color: '#ff9800', bg: 'rgba(255,152,0,0.15)',   live: true  }
 ...
 // Borde y sombra según estado
 const borderColor = isLive
   ? isPenalty ? 'rgba(204,102,255,0.4)'
   : isET      ? 'rgba(255,152,0,0.4)'
   : 'rgba(0,230,118,0.4)'
   : isElimination ? `${stageColor}60` : '#1e3a5f'

 const boxShadow = isLive
   ? isPenalty ? '0 0 20px rgba(204,102,255,0.15)'
   : isET      ? '0 0 20px rgba(255,152,0,0.15)'
   : '0 0 20px rgba(0,230,118,0.1)'
   : isElimination ? `0 0 14px ${stageColor}18` : 'none'

 const hoverShadow = isLive
   ? isPenalty ? '0 0 28px rgba(204,102,255,0.25)'
   : isET      ? '0 0 28px rgba(255,152,0,0.25)'
   : '0 0 28px rgba(0,230,118,0.2)'
   : isElimination ? `0 0 22px ${stageColor}28` : '0 8px 32px rgba(0,191,255,0.12)'

 // Color del score según estado
 const scoreColor = isPenalty ? '#cc66ff' : isET ? '#ff9800' : isLive ? '#00e676' : 'text.primary'
           ...
           <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
             ...
             {status.live && (
               <FiberManualRecord sx={{
                 fontSize: 10,
                 color: isPenalty ? '#cc66ff' : isET ? '#ff9800' : '#00e676',
             ...
             {showScore ? (
               <Box>
                 <Typography variant="h4" sx={{
                   ...
                   '@keyframes scoreGlow': {
                     '0%, 100%': { textShadow: `0 0 8px ${scoreColor}50` },
                     '50%':      { textShadow: `0 0 20px ${scoreColor}90` },
                   },
                 }}>
                   {match.home_score ?? 0}
                 ...
                 {isLive && (
                   <Typography variant="caption" sx={{
                     color: scoreColor, fontWeight: 700, fontSize: '0.65rem',
                   }}>
                     {isPenalty ? 'PENALES' : isET ? 'PRÓRROGA' : 'EN VIVO'}
                   </Typography>
                 )}
               </Box>
             ) : showLiveAnimation ? (
